...
env.addFilter('formatDateTime', (date) =>
  new Date(date).toISOString().slice(0, -1).split('.')[0]
);
...
app.get(['/', '/login','/abc' ], (request, response) =>
  isLoggedin(
    request,
    () => {
...
app.post(['/', '/login'], (request, response) =>
  init(request, (settings) => {
    try {
      ...
      let hashedPassword = cryptography
        .createHash('sha1')
      ...
      if (settings['brute_force_protection'] == 'true') {
        loginAttempts(ip, false, (result) => {
          if (result && result['attempts_left'] <= 1) {
            ...
            response.send(
              'You cannot login right now! Please try again later!'
            );
            return response.end();
          }
        });
          ...
          (error, accounts) => {
            ...
            if (accounts.length > 0) {
              ...
              if (request.body.rememberme) {
                ...
                let hash = accounts[0].rememberme
                  ? accounts[0].rememberme
                  : cryptography
                      .createHash('sha1')
              ...
              connection.query('DELETE FROM login_attempts WHERE ip_address = ?', [
                ip,
              ]);
      ...
      // Handle the error
      console.error(err);
      // Set an appropriate response status code
      response.status(500);
